...
def create_vcon_from_email(body):
    try:
        ...
        vCon.add_dialog_inline_text(
            json.dumps(body), body["email_sent_at"], 0, 0, body["email_mime_type"]
        )
        return vCon
...
def create_vcon_from_sms(body):
    try:
        ...
        vCon.add_dialog_inline_text(
            body["message_body"], body["message_sent_at"], 0, 0, "MIMETYPE_TEXT_PLAIN"
        )
        return vCon
            ...
            async with async_timeout.timeout(10):
                for ingress_list in opts["ingress-list"]:
                    ...
                    try:
                        ...
                        await r.json().set(
                            "vcon:{}".format(vCon.uuid), Path.root_path(), cleanvCon
                        )
