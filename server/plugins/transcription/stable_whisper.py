...
def get_new_attrs(obj_, attr: str):
    if attr == "no_caption_probs":
        return (
            getattr(obj_, attr)
            if hasattr(obj_, "no_caption_probs")
            else getattr(obj_, "no_speech_probs")
        )
    elif attr == "no_caption_prob":
        return (
            getattr(obj_, attr)
            if hasattr(obj_, "no_caption_prob")
            else getattr(obj_, "no_speech_prob")
        )
    elif attr == "no_captions":
        return (
            getattr(obj_, attr)
            if hasattr(obj_, "no_captions")
            else getattr(obj_, "no_speech")
        )
    else:
        raise NotImplementedError(attr)


def check_ascending_sequence(
    seq: Union[List[Union[int, float]], np.ndarray], verbose=True
) -> bool:
    """
...
def check_ascending_sentence_ts(res: (dict, list)) -> bool:
    ...
    return check_ascending_sequence(
        list(chain.from_iterable((float(i["start"]), float(i["end"])) for i in segs))
    )


def check_ascending_word_ts(res: (dict, list)) -> bool:
    cc = group_word_timestamps(
        res["segments"] if isinstance(res, dict) else res, ts_key="word_timestamps"
    )
    return check_ascending_sequence(
        (list(chain.from_iterable((float(i["start"]), float(i["end"])) for i in cc)))
    )
...
def check_is_same_results(
    res0: (dict, list), res1: (dict, list), check_unstable=False
) -> bool:
    """
    ...
    t = set(
        set(
            _reduce(is_equal_ts(a[inner_ts_key], b[inner_ts_key]))
            for a, b in zip(i[ts_key], j[ts_key])
        )
        == {True}
...
def group_word_timestamps(
    ...
    def group_ts(ts_: List[dict], start) -> List[dict]:
        ...
        for w_ts in ts_:
            if first_group:
                if (
                    (not combine_compound or w_ts["word"].startswith(" "))
                    and (w_ts["timestamp"] - first_group[-1]["start"]) >= min_dur
                    and first_group[-1]["end"] < w_ts["timestamp"]
                ):
                    first_group.append(
                        dict(
                            start=first_group[-1]["end"],
                            end=w_ts["timestamp"],
                            text=w_ts["word"],
                        )
                    )
                else:
                    first_group[-1]["end"] = max(
                        first_group[-1]["end"], w_ts["timestamp"]
                    )
                    first_group[-1]["text"] += w_ts["word"]
            else:
                first_group.append(
                    dict(start=start, end=w_ts["timestamp"], text=w_ts["word"])
                )
    ...
    assert set(ts_key in seg for seg in segs) == {
        True
    }, f"input contains missing {ts_key}"

    grouped = (group_ts(seg[ts_key], seg["start"]) for seg in segs)
    return (
        group_zero_duration(list(chain.from_iterable(grouped)))
        if one_group
        else list(grouped)
    )


def tighten_timestamps(
    res: dict, end_at_last_word=True, end_before_period=False, start_at_first_word=False
) -> dict:
    res = deepcopy(res)
    for i in range(len(res["segments"])):
        if start_at_first_word:
            res["segments"][i]["start"] = res["segments"][i]["word_timestamps"][0][
                "timestamp"
            ]
        if (
            end_before_period
            and res["segments"][i]["word_timestamps"][-1] == "."
            and len(res["segments"][i]["word_timestamps"]) > 1
        ):
            res["segments"][i]["end"] = res["segments"][i]["word_timestamps"][-2][
                "timestamp"
            ]
        elif end_at_last_word:
            res["segments"][i]["end"] = res["segments"][i]["word_timestamps"][-1][
                "timestamp"
            ]
...
def results_to_srt(
    ...
    if word_level:
        results_to_word_srt(
            res, srt_path, combine_compound=combine_compound, strip=strip
        )
    else:
...
def results_to_word_srt(
    res: dict, srt_path, combine_compound=False, strip=False, min_dur: float = None
):
    """
    ...
    """
    to_srt(
        group_word_timestamps(res, combine_compound=combine_compound, min_dur=min_dur),
        srt_path,
        strip=strip,
    )


def results_to_token_srt(
    res: dict, srt_path, combine_compound=False, strip=False, min_dur: float = None
):
    """
...
def _get_min_estimation(
    estimations: List[Union[list, np.ndarray]],
    min_: (int, float) = None,
    max_: (int, float) = None,
) -> np.ndarray:
    ...
    estimations = list(
        map(
            lambda est_: np.array(est_) if isinstance(est_, list) else est_, estimations
        )
    )
    prev_min = min_ or 0
...
def _remove_overestimation(
    ...
    if alt_est is not None:
        alt_est = list(
            map(
                lambda est_: np.array(est_) if isinstance(est_, list) else est_, alt_est
            )
        )
    assert x.ndim == 1
    ...
    for i in range(x.shape[-1] - 1, -1, -1):
        if x[i] > max_val or (i > 1 and x[i] < reduce_(x[:i])):  # spikes or dips
            if alt_est is None or alt_est[i] is None:
                ...
                alt_ = alt_est[i][
                    np.logical_and(alt_est[i] < max_val, alt_est[i] > tmp_min)
                ]
...
def _remove_underestimation(
    ...
    for i in range(0, x.shape[-1]):
        if x[i] < min_val or (
            i < max_i_reduce and x[i] > reduce_(x[i + 1 :])
        ):  # dips or spikes
            if alt_est is None or alt_est[i] is None:
                x[i] = min_val
            else:
                tmp_max = (
                    max_val if i >= max_i_reduce else curr_min_max(np.mean(x[i + 1 :]))
                )
                alt_ = alt_est[i][
                    np.logical_and(alt_est[i] > min_val, alt_est[i] < tmp_max)
                ]
...
def _merge_max_min_estimation(
    mx: Union[np.ndarray, List[Union[int, float]]],
        ...
        if prev_min > mn[i]:
            if mn[i] > mx[i]:  # prev_min > mn[i] > mx[i]
                ...
                if prev_min > mx[i]:  # prev_min > mx[i] > mn[i]
                    ...
                    alt_ = alt_est[i][
                        np.logical_and(alt_est[i] > prev_min, alt_est[i] < mx[i])
                    ]
                    mn[i] = (
                        (mx[i] if pref_mx else prev_min)
                        if alt_.shape[0] == 0
                        else alt_[0]
                    )
            ...
            if mx[i] > prev_min:
                if mn[i] > mx[i]:  # mn[i] > mx[i] > prev_min
                    ...
                    alt_ = alt_est[i][
                        np.logical_and(alt_est[i] > mn[i], alt_est[i] < mx[i])
                    ]
                    if alt_.shape[0]:
                        mn[i] = alt_[0]
                    elif pref_mx:
...
def _stabilize_timestamps(
    timestamps: Union[np.ndarray, List[Union[int, float]]],
    alt_timestamps: List[Union[List[Union[int, float]], np.ndarray]] = None,
    max_: (int, float) = None,
    min_: (int, float) = None,
    aggressive=False,
) -> np.ndarray:
    mx = _remove_overestimation(
        timestamps, alt_est=alt_timestamps, max_=max_, min_=min_, aggressive=aggressive
    )
    mn = _remove_underestimation(
        timestamps, alt_est=alt_timestamps, max_=max_, min_=min_, aggressive=aggressive
    )
    return _merge_max_min_estimation(mx, mn, alt_timestamps)


def _stabilize_more_timestamps(
    timestamps: List[Union[list, np.ndarray]],
    max_: (int, float) = None,
    min_: (int, float) = None,
    average=True,
) -> np.ndarray:
...
def stabilize_timestamps(
    segments: Union[List[dict], dict], top_focus=False, aggressive=False, average=True
) -> List[dict]:
    """
    ...
    missing_ts_idx = set(
        map(
            lambda x: None if x[1].get("unstable_word_timestamps") else x[0],
            enumerate(segments),
        )
    ) - {None}
    ...
    if not no_word_timestamps and missing_ts_idx:
        warnings.warn(
            f"Segments {list(missing_ts_idx)} are missing unstable_word_timestamps. "
    ...
    sectioned_segments_timestamps = [
        dict(
            ...
            alt_timestamps=list(
                chain.from_iterable(
                    (s["alt_start_timestamps"], s["alt_end_timestamps"]) for s in segs
                )
            ),
        )
    ...
    sectioned_stab_timestamps = [
        _stabilize_timestamps(**kwargs).reshape(-1, 2)
        for kwargs in sectioned_segments_timestamps
    ]

    for i in range(len(sectioned_segments)):
        for j in range(len(sectioned_segments[i])):
            (
                sectioned_segments[i][j]["start"],
                sectioned_segments[i][j]["end"],
            ) = sectioned_stab_timestamps[i][j]

            if not missing_ts_idx:
                if top_focus:
                    top_word_ts = [
                        ts_["timestamps"][0]
                        for ts_ in sectioned_segments[i][j]["unstable_word_timestamps"]
                    ]
                    alt_word_ts = [
                        ts_["timestamps"][1:]
                        for ts_ in sectioned_segments[i][j]["unstable_word_timestamps"]
                    ]
                    temp_stab_word_ts = _stabilize_timestamps(
                        top_word_ts,
                        alt_word_ts,
                    ...
                    temp_stab_word_ts = _stabilize_more_timestamps(
                        word_ts,
                        max_=sectioned_segments[i][j]["end"],
                        min_=sectioned_segments[i][j]["start"],
                        average=average,
                ...
                temp_stab_word_ts = [
                    {
                        "word": sectioned_segments[i][j]["unstable_word_timestamps"][k][
                            "word"
                        ],
                        "token": sectioned_segments[i][j]["unstable_word_timestamps"][
                            k
                        ]["token"],
...
def add_whole_word_ts(
    tokenizer: Tokenizer,
    segments: Union[List[dict], dict],
    merge_non_space: bool = None,
    prepend_punctuations: Union[List[str], Tuple[str]] = None,
    append_punctuations: Union[List[str], Tuple[str]] = None,
):
    merge_non_space = (
        (tokenizer.language in ["en"] or tokenizer.language is None)
        if merge_non_space is None
        else merge_non_space
    )
    ...
    missing_idx = set(
        -1 if seg.get("word_timestamps") else i for i, seg in enumerate(segments)
    ) - {-1}
    ...
    if missing_idx:
        if len(missing_idx) == len(segments):
            ...
            return
        print(
            f"Some word_timestamps not found, "
    ...
    for seg_idx, seg in enumerate(segments):
        if seg.get("word_timestamps"):
            ...
            for wts_idx in range(1, len(seg["word_timestamps"]) + 1):
                ...
                tokens = [
                    wts["token"] for wts in seg["word_timestamps"][prev_idx:wts_idx]
                ]
                ...
                if temp_whole_word == remaining_text[: len(temp_whole_word)]:
                    ...
                    if (
                        (
                            ...
                            or not whole_word_timestamps
                        )
                        and temp_whole_word not in append_punctuations
                        and not has_prepend
                    ):
    ...
    if failed_idx:
        print(
            f"Failed to add whole-word timestamps to the following segments: {tuple(failed_idx)}"
        )


def _load_audio_waveform(
    audio: Union[str, bytes, np.ndarray, torch.Tensor], h: int, w: int
) -> np.ndarray:
    """
    ...
    try:
        if isinstance(audio, str):
            ...
            if isinstance(audio, bytes):
                ...
                warnings.warn(
                    "A resampled input causes an unexplained temporal shift in waveform image "
        ...
        raise RuntimeError(
            f"Failed to load audio in waveform: {e.stderr.decode()}"
        ) from e
    else:
        if not waveform:
            ...
            add_msg = (
                ...
                if partial_file and isinstance(audio, bytes)
                else ""
            )
            raise RuntimeError(
                f"Failed to load audio in waveform: {err.decode()}" + add_msg
            )
        return np.frombuffer(waveform, dtype=np.uint8).reshape(h, w)


def _remove_lower_quantile(
    waveform: np.ndarray,
...
def _wave_to_ts_filter(
    waveform: np.ndarray, suppress_middle=True, max_index: (list, int) = None
) -> np.ndarray:
    """
...
def transcribe_word_level(
    ...
    **decode_options,
    ...
    if "no_captions_threshold" in decode_options:
        warnings.warn(
            "no_captions_threshold is deprecated. "
            "Please use no_speech_threshold instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def decode_with_fallback(
        segment: torch.Tensor, suppress_ts_mask: Tensor = None
        ...
        for t in temperatures[1:]:
            ...
            if any(needs_fallback):
                options = DecodingOptions(**kwargs, temperature=t)
                retries, r_ts_tokens, r_ts_logits = model.decode(
                    segment[needs_fallback],
                    options,
                    ts_num=ts_num,
                    alpha=alpha,
                    suppress_ts_mask=suppress_ts_mask,
    ...
    def add_segment(
        *,
        ...
        start_timestamps: list = None,
        end_timestamps: list = None,
        word_timestamps: Tensor = None,
        start_ts_logits: list = None,
        end_ts_logits: list = None,
        word_ts_logits: Tensor = None,
    ):
        ...
        if word_timestamps is not None:
            ...
            if word_ts_logits is None:
                word_ts_fields = zip(
                    text_tokens_no_eot, word_timestamps[no_eot_mask], repeat(None)
                )
            else:
                assert word_ts_logits.shape[0] == text_tokens.shape[0]
                word_ts_fields = zip(
                    text_tokens_no_eot,
                    word_timestamps[no_eot_mask],
                    word_ts_logits[no_eot_mask],
                )
        ...
        if print_unstab or (verbose and not stab):
            ...
            if word_timestamps is not None:
                ts_str = (
                    f' ->[{format_timestamp(ts_["timestamps"][0])}] "{ts_["word"].strip()}"'
                    for ts_ in word_timestamps
                )
                print("\n".join(ts_str), end="\n\n")

    if suppress_silence:
        ts_scale = HOP_LENGTH / SAMPLE_RATE / time_precision
        wf = _load_audio_waveform(
            audio_for_mask or audio, 100, int(mel.shape[-1] * ts_scale)
        )
    ...
    while seek < mel.shape[-1]:
        ...
        segment_duration = min(
            float(segment.shape[-1] * HOP_LENGTH / SAMPLE_RATE), remaining_duration
        )
        ...
        if suppress_silence:
            ...
            if (
                remove_background
                and (1 - segment_wf.sum(0).clip(max=1).mean()) < silence_threshold
            ):
                segment_wf = _remove_lower_quantile(
                    segment_wf.astype(np.float32),
                    upper_quantile=upper_quantile,
                    lower_quantile=lower_quantile,
                    lower_threshold=lower_threshold,
                )
            segment_wf = pad_or_trim(segment_wf, 1501)
            suppress_ts_mask = torch.from_numpy(
                _wave_to_ts_filter(
                    segment_wf,
        ...
        result, finalized_ts_tokens, ts_logits = decode_with_fallback(
            segment, suppress_ts_mask=suppress_ts_mask
        )
        ...
        if (
            ...
            for current_slice in consecutive:
                ...
                add_segment(
                    ...
                    start=timestamp_offset + start_timestamp_position * time_precision,
                        ...
                        timestamp_offset + end_timestamp_position * time_precision,
            ...
            if len(timestamps) > 0:
                ...
                last_timestamp_position = min(
                    timestamps[-1].item() - tokenizer.timestamp_begin, segment_max_ts
                )
    ...
    if len(all_segments) > 1 and all_segments[-1]["alt_start_timestamps"] is None:
        all_segments[-1]["alt_start_timestamps"] = all_segments[-2][
            "alt_end_timestamps"
        ]
    ...
    if stab:
        ...
        if verbose:
            ...
            for seg_ in all_segments:
                print(
                    f'[{format_timestamp(seg_["start"])} --> {format_timestamp(seg_["end"])}] "{seg_["text"]}"'
                )
                if seg_["word_timestamps"]:
                    ts_str = (
                        f' ->[{format_timestamp(ts_["timestamp"])}] "{ts_["word"].strip()}"'
                        for ts_ in seg_["word_timestamps"]
                    )
                    print("\n".join(ts_str), end="\n\n")
...
class GreedyDecoderWordLevel(GreedyDecoder):
    ...
    def _suppress_ts(self, logits: Tensor):
        _suppress_ts(
            logits[:, self.timestamp_begin :], suppress_ts_mask=self.suppress_ts_mask
        )

    def update_with_ts(
        self, tokens: Tensor, logits: Tensor, sum_logprobs: Tensor, ts: Tensor
    ) -> Tuple[Tensor, bool]:
        self.ts = ts
...
class BeamSearchDecoderWordLevel(BeamSearchDecoder):
    ...
    def update_with_ts(
        ...
        for previously_finished, newly_finished, prev_ts_ls, new_ts_ls in zip(
    ...
    def finalize(self, preceding_tokens: Tensor, sum_logprobs: Tensor):
        ...
        for i, (sequences, ts_) in enumerate(
            zip(self.finished_sequences, self.finished_ts_ls)
        ):
            if (
...
class DecodingTaskWordLevel(DecodingTask):
    def __init__(self, *args, **kwargs):
        ...
        if hasattr(self.decoder, "beam_size"):
            self.decoder = BeamSearchDecoderWordLevel(
                self.decoder.beam_size,
                self.decoder.eot,
                self.inference,
            ...
            self.decoder = GreedyDecoderWordLevel(
            ...
            for i in range(self.sample_len):
                if self.alpha:
                    logits = self.inference.logits(
                        tokens,
                        audio_features
                        * (torch.rand_like(audio_features) * self.alpha + 1),
                    )
                else:
                    logits = self.inference.logits(tokens, audio_features)

                if (
                    i == 0 and get_new_attrs(self.tokenizer, "no_captions") is not None
                ):  # save no_speech_probs
                    probs_at_sot = logits[:, self.sot_index].float().softmax(dim=-1)
                    no_speech_probs = probs_at_sot[
                        :, get_new_attrs(self.tokenizer, "no_captions")
                    ].tolist()
                ...
                tokens, completed = self.decoder.update_with_ts(
    ...
    def run(
        ...
        ts: List[List[Tensor]] = [
            [t[:, : tokens[i][j].shape[-1]] for j, t in enumerate(s)]
        ...
        return [
            DecodingResult(
                ...
                **(
                    dict(no_caption_prob=no_speech_prob)
                    if hasattr(DecodingResult, "no_caption_prob")
                    else dict(no_speech_prob=no_speech_prob)
                ),
...
def decode_word_level(
    ...
    result, ts = DecodingTaskWordLevel(
        model,
        options,
        ts_num=ts_num,
        alpha=alpha,
